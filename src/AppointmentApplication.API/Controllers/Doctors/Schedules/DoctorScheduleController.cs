...
public sealed class DoctorScheduleController(ISender sender, LinkService linkService) : ApiController
{
    ...
    [HttpGet("me", Name = "GetMyDoctorSchedules")]
    [Authorize(Roles = Roles.Doctor)]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    [MapToApiVersion("0.1")]
    [EndpointSummary("Get current doctor's schedules.")]
    [EndpointDescription("Retrieves schedules for the currently authenticated doctor.")]
    [EndpointName("GetMyDoctorSchedules")]
    ...
    private List<LinkDto> CreateLinks(Guid doctorId, string id, string? fields)
    {
        ...
        // Add my-schedules link if the authenticated user is a doctor
        // if (currentUserService.IsInRole(Roles.Doctor))
        // {
        //     links.Add(linkService.Create(nameof(GetMyDoctorSchedules), "my-schedules", HttpMethods.Get));
        // }
